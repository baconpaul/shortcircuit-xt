    ...
    if (wsi)
    {
            ...
            else
            {
                ...
                for (auto e : els)
                {
                    ...
                    if (e->getCompoundElement().has_value())
                        compounds.push_back(
                            {*e->getCompoundElement(), loc.root, loc.keyLo, loc.keyHi, loc.velLo, loc.velHi});
        ...
        else if (wsi->getCompoundElement().has_value())
        {
            ...
            sendToSerialization(cmsg::AddCompoundElementWithRange(
                {*wsi->getCompoundElement(), loc.root, loc.keyLo, loc.keyHi, loc.velLo, loc.velHi}));
        }
...
void MappingDisplay::filesDropped(const juce::StringArray &files, int x, int y)
{
    ...
    for (auto f : files)
    {
        ...
        if (inst.empty())
            samples.push_back({f.toStdString(), loc.root, loc.keyLo, loc.keyHi, loc.velLo, loc.velHi, false});
        else
            promptForMultiInstrument(inst);
    }
    if (!samples.empty())
